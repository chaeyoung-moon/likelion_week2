...
void displayDis(struct coordinate* a, struct coordinate* b, int len) {
	for (int i = 0; i < len; i++) {
		int distanceSec = sqrt((a[i].latitude.totalSec - b[i].latitude.totalSec)*(a[i].latitude.totalSec - b[i].latitude.totalSec) 
				+ (a[i].longitude.totalSec - b[i].longitude.totalSec)*(a[i].longitude.totalSec - b[i].longitude.totalSec));
